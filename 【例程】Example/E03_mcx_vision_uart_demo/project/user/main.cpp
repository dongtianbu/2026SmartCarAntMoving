...
// *************************** 例程测试说明 ***************************
// 1.使用DAP下载器连接模块，使用TTL接到模块的用户串口上
// 
// 2.MCX Vision下载本例程，
// 
// 3.复位核心板（需要手动复位） 
// 
// 4.模块通过DEBUG串口和用户串口输出字符串
//
// 5.使用电脑上位机打开DAP的串口号和TTL串口号，向模块发送字符，模块会返回相同的字符
// **************************** 代码区域 ****************************
...
{
    zf_debug_printf("debug_get:%c\r\n", c);
}

void zf_user_uart_callback(uint8 c)
{
    zf_user_printf("user_get:%c\r\n", c);
}
// **************************** 代码区域 ****************************
#if defined(__cplusplus)
}
