...
if ([string]::IsNullOrWhiteSpace($Port)) {
    ...
    $Port = Read-Host "Enter CarA serial port (example: COM7)"
}
...
try {
    Write-Host "== CarA deploy start =="
    Write-Host "Project: $projectRoot"
    Write-Host "Port: $Port"
    Write-Host "Mode: $Mode"
    Write-Host ""

    $stdoutPath = Join-Path $env:TEMP "flash_car_a_stdout.txt"
    $stderrPath = Join-Path $env:TEMP "flash_car_a_stderr.txt"
    ...
    Write-Host ""
    Write-Host "== CarA deploy success =="
} finally {
    Remove-Item $stdoutPath, $stderrPath -ErrorAction SilentlyContinue
    Pop-Location
}
