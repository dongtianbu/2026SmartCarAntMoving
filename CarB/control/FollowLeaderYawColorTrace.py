...
YAW_FOLLOW_LEADER_ENABLED    = False             # True 时跟随主车 yaw，False 时改为固定目标角并开启电脑调试
YAW_FIXED_TARGET_DEG         = 0.0               # 固定 yaw 模式下的目标角度
TUNING_RX_TEXT_BUF_LIMIT     = 512               # 无线调参文本缓冲区上限
TUNING_REPLY_ENABLED         = True              # 是否回发调参执行结果
CAMERA_LOOK_RIGHT_ENABLED    = True              # True 表示摄像头朝车右侧，需要做坐标映射
AI_TUNING_CONTROL_ENABLED    = True              # True 时允许 AI_STOP / AI_START / AI_DEVIATE 等命令
AI_CONTROL_PAUSED_ON_START   = True              # True 时上电后先暂停 AI 控制，等待 AI_START
AI_METRIC_ENABLED            = True              # True 时周期输出 AI_METRIC 数据
AI_METRIC_WIRELESS_ENABLED   = True              # True 时同时从无线串口回发 AI_METRIC 数据
...
# PID_KP=0.18                           # 设置 yaw 比例系数
# COLOR_KP_X=0.033                      # 设置横向比例系数
# YAW_FIXED_TARGET_DEG=0                # 设置固定 yaw 目标
# ENABLE_SERIAL_LOG=1                   # 布尔量支持 1/0、TRUE/FALSE、ON/OFF、YES/NO
# CAMERA_LOOK_RIGHT_ENABLED=1           # 开启“摄像头朝车右侧”模式，0 或 OFF 时恢复朝前
# AI_STOP                               # 暂停 AI 控制
# AI_START                              # 恢复 AI 控制
# AI_DEVIATE 12 1200                    # 先偏离 12 度，持续 1200ms，再自动回到固定 yaw 目标
# AI_PERTURB 45 20 45 1200              # 按 45 度方向平移，速度 20，同时以 45 旋转 1200ms
# AI_STATUS                             # 查询当前 AI 控制状态
#
# 可调参数列表
...
class WirelessTuningSession:
    ...
    def start(self):
        ...
        if self.config["tuning_reply_enabled"]:
            ...
            self.radio.send_line("CMD: NAME=VALUE | GET NAME | LIST | AI_STOP | AI_START | AI_DEVIATE deg ms | AI_PERTURB angle speed rotate ms")
    ...
    def _handle_line(self, line):
        ...
        if line_upper == "HELP":
            self._reply("CMD: NAME=VALUE | GET NAME | LIST | AI_STOP | AI_START | AI_DEVIATE deg ms | AI_PERTURB angle speed rotate ms | AI_STATUS")
            return
                ...
                if tick % config["status_print_every"] == 0:
                    ...
                    if config["ai_metric_enabled"] and not result["ai_control_paused"]:
                        ...
                        if (
                            ...
                            controller.tuner._reply(metric_line)
