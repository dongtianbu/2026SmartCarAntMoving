...
class ImuSensorVertical:
    ...
    def calibrate(self):
        ...
        # 在做姿态计算前，先把竖装 IMU 的数据映射到逻辑底盘坐标系。
        raw_ax = self._sign_acc[0] * self._imu_data[0] / self._acc_lsb_per_g
        raw_ay = self._sign_acc[1] * self._imu_data[1] / self._acc_lsb_per_g
        raw_az = self._sign_acc[2] * self._imu_data[2] / self._acc_lsb_per_g
        ...
        print("Vertical mount: original z -> new -y, original y -> new z")
        return True
    ...
    def update(self):
        ...
        raw_ax = self._sign_acc[0] * ax_raw / self._acc_lsb_per_g
        raw_ay = self._sign_acc[1] * ay_raw / self._acc_lsb_per_g
        raw_az = self._sign_acc[2] * az_raw / self._acc_lsb_per_g

        # 竖装时逻辑 y/z 轴与平装方案不同，这里先完成轴向转换。
        ax = raw_ax
        ay = -raw_az
        az = raw_ay
        ...
        # 互补滤波：加速度计负责长期稳定，陀螺仪负责短时响应。
        roll_acc = math.degrees(math.atan2(self._ay_f, self._az_f))
        pitch_acc = math.degrees(math.atan2(-self._ax_f,
                                             math.sqrt(self._ay_f ** 2 + self._az_f ** 2)))

        self._roll = self._comp_alpha * (self._roll + gyro_x_new * dt) + (1.0 - self._comp_alpha) * roll_acc
        self._pitch = self._comp_alpha * (self._pitch + gyro_y_new * dt) + (1.0 - self._comp_alpha) * pitch_acc

        # 这里的 yaw 完全依赖陀螺积分，更适合短时使用。
