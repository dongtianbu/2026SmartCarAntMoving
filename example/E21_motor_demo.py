...
motor_3 = MOTOR_CONTROLLER(MOTOR_CONTROLLER.PWM_D4_DIR_D5  , 1000, duty = 0, invert = False)  # 降低频率到 1000Hz，便于电机响应
...
motor_dir = 1
motor_duty = 0
motor_duty_max = 10000  # 最大占空比到 100%

while True:
    time.sleep_ms(200)  # 增加延迟，便于观察
    
    if motor_dir:
        motor_duty = motor_duty + 200  # 减小步长，更细调节
        if motor_duty >= motor_duty_max:
            motor_dir = 0
    else:
        motor_duty = motor_duty - 200
        if motor_duty <= -motor_duty_max:
            motor_dir = 1
